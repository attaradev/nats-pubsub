...
RSpec.describe NatsPubsub::Subscribers::MessageProcessor do
  let(:jts) { double('jetstream') }
  let(:handler) { double('handler') }
  let(:dlq) { instance_double(NatsPubsub::Subscribers::DlqHandler) }
  ...
  context 'when handler succeeds' do
    ...
    it 'acks the message' do
      ...
      expect(dlq).not_to receive(:publish_to_dlq)
  ...
  context 'when handler raises an unrecoverable error' do
    ...
    it 'acks and publishes to the DLQ' do
      ...
      expect(msg).to receive(:ack)
      expect(dlq).to receive(:publish_to_dlq) do |m, ctx, error_context:|
        expect(m).to eq(msg)
        expect(ctx.event_id).to eq('abc-123')
        expect(error_context.reason).to eq('unrecoverable')
        expect(error_context.error_class).to eq('ArgumentError')
        expect(error_context.error_message).to eq('bad')
        true # Return true to simulate successful publish
      end.and_return(true)
      processor.handle_message(msg)
    end
  end
