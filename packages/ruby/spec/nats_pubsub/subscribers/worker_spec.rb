...
RSpec.describe NatsPubsub::Subscribers::Worker do
  let(:jts) { double('jetstream') }
  let(:subscription) { double('subscription') }
  let(:sub_mgr) { instance_double(NatsPubsub::Subscribers::SubscriptionManager) }
  let(:processor) { instance_double(NatsPubsub::Subscribers::MessageProcessor) }

  before do
    NatsPubsub.reset!
    NatsPubsub.configure { |c| c.destination_app = 'dest' }
    allow(NatsPubsub::Connection).to receive(:connect!).and_return(jts)
    allow(NatsPubsub::Subscribers::SubscriptionManager).to receive(:new).and_return(sub_mgr)
    allow(NatsPubsub::Subscribers::MessageProcessor).to receive(:new).and_return(processor)
  ...
  describe 'initialization' do
    it 'ensures and subscribes the consumer' do
      ...
      expect(NatsPubsub::Subscribers::SubscriptionManager)
